export default {
  name: "enable",
  description: "Enable a bot feature (owner/mod only)",
  async execute(msg, { sock, args, isOwner, isMod, featureToggles, saveFeatureToggles }) {
    ...
    if (args.length === 0) {
      return await sock.sendMessage(from, {
        text: `❌ Please specify a feature to enable.\nAvailable features:\n${Object.keys(featureToggles).join("\n")}`,
      }, { quoted: msg });
    }
    ...
    saveFeatureToggles();
    await sock.sendMessage(from, { text: `✅ Feature "${feature}" has been enabled.` }, { quoted: msg });
  },
};
